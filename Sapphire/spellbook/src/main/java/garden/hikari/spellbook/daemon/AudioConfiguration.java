...
public class AudioConfiguration {
    ...
    public static AudioProcessor processConfiguration(String configString) throws Exception{
        JSONObject config = new JSONObject().getJSONObject(configString);
    ...
    public static AudioProcessor getDefaultProcessor(Context context, String type) throws Exception{
        ...
        if(type.equals(AudioProcessor.WAKE)){
            ...
            FileReader fileReader = new FileReader(assets.openFd("wakeword.json").getFileDescriptor());
            BufferedReader bufferedReader  = new BufferedReader(fileReader);
            bufferedReader.lines().forEach(configString::concat);
            JSONObject jsonObject = new JSONObject(configString);
        }else if(type.equals(AudioProcessor.GENERAL)){
            assets.open("general.json");
        ...
        processor.setModel(context.getApplicationContext());
